...
def lowest_cost(game_status: _GameStatus) -> Tuple[int, List[Tuple[_Amphipod, _Position, _Position]]]:
    ...
    for amphipod, position, at_home in game_status:
        ...
        if position[0] == 0:
            ...
            new_position = (1, home_column) if (2, home_column) in all_amphipod_positions else (2, home_column)
            if _move_is_blocked(
                all_amphipod_positions=all_amphipod_positions, start_position=position, end_position=new_position
            ):
                continue

            if (2, home_column) in all_amphipod_positions:
                if game_status[[x for _, x, _ in game_status].index((2, home_column))][0][0] != amphipod[0]:
                    # The deeper room space is occupied by a wrong type.
                    continue
...
def _move_is_blocked(
    ...
    # If moving into a room, the end position must not be occupied.
    # If moving out from or into the deeper space in the room, then the shallower part of the room must not be blocked.
    if (
        end_position in other_amphipod_positions
        or start_position[0] == 2
        and (1, start_position[1]) in other_amphipod_positions
        or end_position[0] == 2
        and (1, end_position[1]) in other_amphipod_positions
    ):
        return True
