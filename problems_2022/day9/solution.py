...
def _tail_positions(start_position: _Position, instructions: List[_Instruction], num_knots: int) -> List[_Position]:
    ...
    for direction, num_steps in instructions:
        for _ in range(num_steps):
            ...
            _move_one_step(position=head_position, direction=direction)

            # Recursively find the next knot's position based on the previous knot's position.
            for knot_index in range(1, num_knots):
                _update_knot_position(
                    this_knot_position=knot_positions[knot_index], previous_knot_position=knot_positions[knot_index - 1]
                )
...
def _move_one_step(position: np.ndarray, direction: str) -> None:
    if direction == "R":
        position[1] += 1
    elif direction == "L":
        position[1] -= 1
...
def _update_knot_position(this_knot_position: np.ndarray, previous_knot_position: np.ndarray) -> None:
    """Updates `this_knot_position` in-place, assuming `previous_knot_position` has already been updated."""
    # Only need to move if in at least one of the axes, the distance is 2.
    if np.max(np.abs(previous_knot_position - this_knot_position)) <= 1:
        return

    axes_with_distance_2 = np.where(np.abs(previous_knot_position - this_knot_position) == 2)[0]
    for axis in range(2):
        if axis in axes_with_distance_2:
            ...
            this_knot_position[axis] = int((this_knot_position[axis] + previous_knot_position[axis]) / 2)
        else:
            this_knot_position[axis] = previous_knot_position[axis]
